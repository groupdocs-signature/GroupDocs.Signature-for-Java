...
import com.groupdocs.ui.signature.ktor.config.ApplicationConfig
import com.groupdocs.ui.signature.ktor.config.SignatureConfig
import com.groupdocs.ui.signature.ktor.di.ModulesInjection
...
fun main(args: Array<String>) {
    ...
    val comparerConfig by lazy { extractSignatureConfig() }
...
fun extractSignatureConfig(): SignatureConfig {
    val hocon = Hocon {
        useConfigNamingConvention = false
    }
    return hocon.decodeFromConfig(SignatureConfig.serializer(), ConfigFactory.load("comparer.conf"))
}
...
fun Application.setGroupdocsLicense(licensePath: String) {
    try {
        ...
        log.warn("Can not verify Signature license!", e)
    }
}
